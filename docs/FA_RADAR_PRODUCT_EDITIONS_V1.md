...
The current build slice is `0.1.57`. It produces Free and Pro DLLs from the
same source file:
...
buttons, and text fields, but 0.1.57 intentionally keeps Free sparse: Radar
...
The `0.1.56` pass repairs the live producer/consumer join for the compatible v8
tracked-hand state packet and removes the stale atom-host restriction from the
Room Compass toggle already exposed by the scene/session UI.
The `0.1.57` pass preserves those visible surfaces while replacing timer-driven
scene/selection/recorder work with VaM lifecycle dirtying, bounded candidate
reconciliation, moving/dirty marker subsets, reusable camera buffers, and
change-driven materials and status storables.
...
- no recurring runtime file I/O, reflection, broad JSON object serializers, repo-local
