...
# 0.1.54 audit anchors. The live values are read from config/fa_radar.version.json.
# Free: FA_RADAR_FREE -> fa_radar.free.0.1.54.dll, FrameAngelDev.Radar.1.var
...
if (-not $SkipCompile.IsPresent) {
    ...
    $references = @(
        ...
        (Get-RequiredReference -ManagedDir $resolvedVamManagedDir -Name "UnityEngine.JSONSerializeModule.dll"),
