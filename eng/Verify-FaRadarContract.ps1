...
if (-not (Test-Path -LiteralPath $cuaPresetPath -PathType Leaf)) {
    ...
    $requiredCuaPresetSnippets = @(
        ...
        '"id" : "PluginManager"',
        '"plugin#0" : "Custom/Plugins/fa_radar.pro.0.1.20.dll"',
...
if (-not (Test-Path -LiteralPath $versionPath)) {
    ...
    if ($version.version -ne "0.1.20") {
        Add-Failure "Version config must declare version 0.1.20."
    }
    if ($version.branch -ne "codex/0.1.20-built-in-grab-parent") {
        Add-Failure "Version config branch must match codex/0.1.20-built-in-grab-parent."
    }
...
if ($ValidateLiveDeploy.IsPresent) {
    ...
    foreach ($root in $roots) {
        $expectedDlls = @(
            (Join-Path $root "Custom\Plugins\fa_radar.free.0.1.20.dll"),
            (Join-Path $root "Custom\Plugins\fa_radar.pro.0.1.20.dll")
        )
...
if (Test-Path -LiteralPath $pluginPath) {
    ...
    if ($plugin.Contains("UpdateLastSelectedBlip(viewer);")) {
        Add-Failure "Previous-selection rendering must stay disabled in 0.1.20."
    }
